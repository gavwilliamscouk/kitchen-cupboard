...
export default function InventoryScreen() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("cupboard_collapsed_categories");
    if (saved) {
      try { setCollapsedCategories(JSON.parse(saved)); } catch (e) {}
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("cupboard_collapsed_categories", JSON.stringify(collapsedCategories));
  }, [collapsedCategories]);
  ...
  const filteredItems = items
    .filter(item => item.inCupboard !== false)
  ...
  const handleOpenAddModal = (cat: string = "") => {
    setEditingItem(null);
    setDefaultCategoryForModal(cat);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="flex gap-3 items-center w-full">
        ...
        {sortBy === "category" && Object.keys(groupedItems).length > 0 && (
          <button
            onClick={() => {
              const allKeys = Object.keys(groupedItems);
              const allCollapsed = allKeys.every(k => collapsedCategories[k]);
              if (allCollapsed) {
          ...
          >
            {Object.keys(groupedItems).every(k => collapsedCategories[k]) ? "Expand All" : "Collapse All"}
          </button>
        )}
      </div>
